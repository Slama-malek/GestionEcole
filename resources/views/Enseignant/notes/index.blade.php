...
@endif
        <div class="heading-layout1">
            ...
            <div class="dropdown">
                <h4>Classe :<strong style="color:#8c4a99"> </strong> <strong style="color:#042954">{{$eleve->nom_c}}</strong> <br> Groupe : <strong style="color:#042954">  {{$eleve->nom_g}} </strong> </h4>
            </div>
        </div>
        ...
        <h5  class="text-gray-700 uppercase font-bold"> Matiére : <strong style="color:#f8a22f">{{$matiere->nom_m}}</strong> </h5>
                ...
                <tbody>
                    <tr>
                        ...
                        <td >{{$item->id}}</td>
                         ...
                         <td c >
                            <form action="{{ route('note',[$item->id_c,$item->id_g,$matiere->id_m]) }}" method="POST">
                                @csrf
                            <input type="hidden" id="eleve" name="eleve[{{ $item->id }}]" value="{{$item->id}}">
                            <input type="hidden" id="coef" name="coef" value="{{$matiere->id_coef}}">
                            <input type="hidden" id="ens" name="ens" value="{{Auth::user()->id}}">
                           
                            <div class="col-lg-2">
                            <input type="text" name="note[{{ $item->id }}]" placeholder="saisir note" class="form-control ">
